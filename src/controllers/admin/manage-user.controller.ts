    ...
    .aggregate([
      {
        $facet: {
          ...
          newUsers: [{ $match: { create_at: { $gte: startOfToday, $lte: endOfToday } } }, { $count: 'count' }]
        }
      }
    ])
    .toArray()
...
export const getListUsersController = async (req: Request, res: Response) => {
  ...
  const result = await userService.getListUsers({ page, limit, status, search, sort, startDate, endDate })
...
export const logoutUserController = async (req: Request, res: Response) => {
  ...
  await databaseService.refreshTokens.deleteMany({ user_id: new ObjectId(userId) })
